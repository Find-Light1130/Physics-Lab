const App = {
    modules: [], container: null, currentExpId: null, renderRequested: false,
    init() {
        this.container = document.getElementById('app');
        this.bindThemeToggle();
        const path = window.location.pathname.replace(/^\/+/, '');
        this.handleRoute(path === '' ? 'main' : path);
        window.addEventListener('popstate', (e) => this.handleRoute(window.location.pathname.replace(/^\/+/, '') || 'main', false));
        document.addEventListener('click', (e) => {
            const link = e.target.closest('[data-link]');
            if(link) { e.preventDefault(); const href = link.getAttribute('href'); history.pushState({}, '', href); this.handleRoute(href.replace(/^\/+/, '') || 'main'); }
        });
    },
    registerModule(module) { this.modules.push(module); },
    getModule(id) { return this.modules.find(m => m.id === id); },
    handleRoute(route, pushState = true) {
        const backBtn = document.getElementById('back-home-btn');
        if(route === 'main') { backBtn.classList.add('hidden'); this.renderHome(); }
        else {
            const module = this.getModule(route);
            if(module) { backBtn.classList.remove('hidden'); this.renderExperiment(module); }
            else { history.replaceState({}, '', '/main'); this.renderHome(); backBtn.classList.add('hidden'); }
        }
    },
    goHome() { history.pushState({}, '', '/main'); this.handleRoute('main'); },
    renderHome() {
    ...
    renderExperiment(module) {
        ...
        const state = module.createState(); window._expState = state; this.currentExpId = module.id;
        this.container.innerHTML = `
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeInUp">
                <div class="lg:col-span-2"><div class="acrylic rounded-2xl p-6">
                    <div class="flex items-center justify-between mb-4 flex-wrap gap-2"><h2 class="text-lg font-semibold text-main">${module.title}</h2>
                    ...
                    <canvas id="expCanvas" width="800" height="450" class="w-full rounded-xl shadow-inner"></canvas>
                ...
                <div class="space-y-4 max-h-[660px] overflow-y-auto scrollbar-thin pr-2 custom-scrollbar">
                    ...
                    <div class="acrylic rounded-2xl p-5"><h3 class="font-semibold mb-4 flex items-center gap-2 text-main"><svg class="w-5 h-5 text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>实时数据</h3><div id="data-container" class="space-y-2 divide-y divide-border-color/50"></div></div>
        ...
        `;
        document.querySelectorAll('.speed-btn').forEach(btn => { btn.addEventListener('click', function() { document.querySelectorAll('.speed-btn').forEach(b => b.classList.remove('active-speed')); this.classList.add('active-speed'); if(window._expState && window._expState.setSpeed) window._expState.setSpeed(parseFloat(this.dataset.speed)); }); });
        state.init(); state.draw(); if(state.onMount) state.onMount();
    },
    bindThemeToggle() {
        ...
        toggleBtn.addEventListener('click', function() { document.body.classList.toggle('dark'); localStorage.setItem('theme', document.body.classList.contains('dark') ? 'dark' : 'light'); });
    }
};
